...
calc_gpp <- function( fpar, ppfd, vpd, alpha, tair, co2, patm, elv, nxtgn=FALSE ){
  #-----------------------------------------------------------------------
...
calc_m_nxtgn <- function(ca, gs, d, k, ns, fa, beta){
  #-----------------------------------------------------------------------
...
chi_approx <- function( temp, vpd, elv ){
  #-----------------------------------------------------------------------
...
density_h2o <- function( self, tc, p ){
  #-----------------------------------------------------------------------
...
viscosity_h2o <- function( tc, p ) {
  ...
  for (i in 1:6){
    ...
    coef2 <- 0.0
    for (j in 1:7){
      coef2 <- coef2 + h_array[j,i] * (rbar - 1.0)^j
    }
    mu1 <- mu1 + coef1 * coef2    
  }
  mu1 <- exp( rbar * mu1 )
...
## Pressure at elevation (450 m a.s.l.)
patm <- calc_patm(450.0)

## CALCULATE GPP for each day where data is available
## with:
##    - fapar = 1
...
gpp <- rep( NA, nmonth )
for ( moy in 1:nmonth ){
  gpp[moy] <- calc_gpp( fpar=1, ppfd=df.ppfd$ppfd[moy], vpd=mvpd[moy], alpha=1.26, tair=mtemp[moy], co2=360, patm=patm, 450, nxtgn=FALSE  )
}

## CALCULATE K
## Michaelis-Menten coefficient (Pa)
kC <- sapply( mtemp, FUN = function(x) calc_k(x, patm) )

## CALCULATE VISCOSITY
visc <- rep( NA, nmonth )
for ( moy in 1:nmonth ){
  visc[moy] <- calc_k( mtemp[moy], patm )
}
...
plot( 1:nmonth, kC, type="l", col="red" )
for (idx in 1:dim(indata)[1]){
