...
source( paste( myhome, "sofun/getin/get_pointdata_fapar_modis.R", sep="" ) )
...
siteinfo <- read.csv( paste( myhome, "sofun/input_pmodel_test_sofun/siteinfo_pmodel_test_sofun.csv", sep="" ) )
...
path_fapar_allsites_csv <- paste( myhome, "sofun/input_pmodel_test_sofun/sitedata/fapar/fapar_modis_allsites.csv")
...
for (idx in seq(nsites)){
  ...
  dirnam_fapar_csv <- paste( myhome, "sofun/input_pmodel_test_sofun/sitedata/fapar/", sitename, "/", sep="" )
  ...
  if (!file.exists(filnam_fapar_csv)){

    idx <- 0
    for (yr in fapar_year_start:fapar_year_end){

      ##--------------------------------------------------------------------
      ## get annual fAPAR data
      ##--------------------------------------------------------------------
    ...
    ##--------------------------------------------------------------------
    ## add dummy years 2000 and 2014, use median of all available years by month
    ##--------------------------------------------------------------------
    tmp <- rep(NA,nmonth)
    for (imo in 1:nmonth){
  ...
  for (yr in unique(df_fapar$year)){
    ...
    dirnam <- paste( paste( myhome, "sofun/input_pmodel_test_sofun/sitedata/fapar/"), sitename, "/", as.character(yr), "/", sep="" )
